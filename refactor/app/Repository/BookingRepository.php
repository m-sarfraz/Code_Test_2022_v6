...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        // if user not found we must handle the situation rather than empty response or error  
        // try catch blocks are important to hanle the data eror or if API is being made, it is necessary to use statusCode and message as string to send as response 
        // env variables must not be directly acessible rather it must be there a class and it can be accessed with. 
        try {
...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == config('class.CUSTOMER_ROLE_ID')) {
            $cuser = $user;
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function jobToData($job)
    {
        ...
        if ($job->certified != null) {
            ...
            // this will be easy with switch statement 
            if ($job->certified == 'both') {
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        foreach ($translators as $translator) {
            ...
            $status = SendSMSHelper::send(config('class.SMS_NUMBER'), $translator->mobile, $message);
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        if (config('class.APP_config') == 'prod') {
            $onesignalAppID = config('class.app.prodOnesignalAppID');
            $onesignalRestAuthKey = sprintf("Authorization: Basic %s", config('class.app.prodOnesignalApiKey'));
        } else {
            $onesignalAppID = config('class.app.devOnesignalAppID');
            $onesignalRestAuthKey = sprintf("Authorization: Basic %s", config('class.app.devOnesignalApiKey'));
        }
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $translator_type = ($job_type == 'paid') ? 'professional' :
        (($job_type == 'rws') ? 'rwstranslator' :
            (($job_type == 'unpaid') ? 'volunteer' : null));

        $joblanguage = $job->from_language_id;
        $gender = $job->gender;
        $translator_level = [];

        // SWITCH STATEMENT SHOULD BE BEST OPTIOPN HERE INSTEAD OF EXISINTG CODE.
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    private function changeStatus($job, $data, $changedTranslator)
    {
        ...
        // mapping here  would be best option here status and methods


        if ($old_status != $data['status']) {
            $statusChangedMethods = [
            ...
            $methodName = $statusChangedMethods[$job->status] ?? null;
            $statusChanged = $methodName ? $this->$methodName($job, $data, $changedTranslator) : false;
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == config('class.SUPERADMIN_ROLE_ID')) {
            $allJobs = Job::query();
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        // here wahere statemtns can bemade as a single array like ['column' => 'value'] 
        // separate order by filter funciotn can be made to handle this and where we pass column and value and return the result. 
    ...
    public function ignoreExpiring($id, $columnName)
    {
        $job = Job::find($id);
        $job->columnName = 1;
        $job->save();
